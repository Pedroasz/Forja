...
create temporary table pg_temp.v41c_verify_results
on commit drop
as
with
functions as (
...
checks as (
  ...
  union all select '137_dedupe_constraint_normalized',exists(select 1 from notification_constraints where conname='user_notifications_dedupe_key_check' and compact like '%dedupe_key=btrim(dedupe_key)%' and compact like '%char_length(btrim(dedupe_key))between1and200%'),'dedupe_key is stored trimmed and bounded'
...
warning_checks as (
  select '160_inactive_relationship_history' check_name,true passed,'DATA' category,'WARNING' severity,historical_assignments_on_inactive_relationships::text found_value,'0 preferred' expected_value,'historical assignments linked to inactive relationships are retained but should be reviewed' details from data_metrics
  union all select '161_expired_notifications_retained',true,'DATA','WARNING',expired_notifications::text,'0 preferred','expired notifications are retained for history but excluded from reads' from data_metrics
),
all_checks as (
...
select
  ...
  count(*) filter (where severity = 'WARNING') over () as warning_checks,
  case
    when bool_or(severity = 'CRITICAL' and passed is not true) over () then 'FAIL'
    when bool_or(severity = 'WARNING') over () then 'WARN'
    else 'PASS'
