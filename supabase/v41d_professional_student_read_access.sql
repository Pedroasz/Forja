...
-- V4.1D exposes bounded, read-only RPCs. It does not change source-table grants or RLS.
...
create or replace function public.assert_professional_student_read_access_v41d(
...
begin
  ...
  if target_relationship_id is null or target_student_user_id is null then
    raise exception 'relationship_and_student_required' using errcode = '22023';
  end if;

  if not (
    (target_professional_type = 'trainer' and target_required_scope in ('view_workout_executions', 'view_evolution'))
    or (target_professional_type = 'nutritionist' and target_required_scope in ('view_nutrition_logs', 'view_evolution'))
  ) then
  ...
  where relationship.id = target_relationship_id
    ...
    and relationship.student_user_id = target_student_user_id
...
declare
  response jsonb;
begin
  perform public.assert_professional_student_read_access_v41d(
  ...
  with candidates as (
    ...
    where workout.user_id = target_student_user_id
      and workout.workout_date >= target_start_date
      and workout.workout_date <= target_end_date
      and (
        target_cursor_date is null
        or (workout.workout_date, workout.id) < (target_cursor_date, target_cursor_id)
      )
  ...
  select pg_catalog.jsonb_build_object(
    'items', coalesce((
      ...
      ) order by item.workout_date desc, item.id desc)
      from visible item
    ), '[]'::jsonb),
...
begin
  ...
  with candidates as (
    ...
    where meal.user_id = target_student_user_id
      and meal.meal_date >= target_start_date
      and meal.meal_date <= target_end_date
      and (
        target_cursor_date is null
        or (meal.meal_date, meal.id) < (target_cursor_date, target_cursor_id)
      )
  ...
  select pg_catalog.jsonb_build_object(
    'items', coalesce((
      ...
      ) order by item.meal_date desc, item.id desc)
      from visible item
    ), '[]'::jsonb),
    'hasMore', (select has_more from page_state),
    'nextCursor', case when (select has_more from page_state) then (
...
declare
  response jsonb;
begin
  if target_professional_type not in ('trainer', 'nutritionist') then
  ...
  with candidates as (
    select
      ...
      evolution.left_arm,
      evolution.right_arm,
      evolution.left_thigh,
      evolution.right_thigh,
      evolution.notes
    from public.evolution evolution
    where evolution.user_id = target_student_user_id
      and evolution.record_date >= target_start_date
      and evolution.record_date <= target_end_date
      and (
        target_cursor_date is null
        or (evolution.record_date, evolution.id) < (target_cursor_date, target_cursor_id)
      )
  ...
  select pg_catalog.jsonb_build_object(
    'items', coalesce((
      select pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
        ...
        'hip', item.hip,
        'chest', item.chest,
        'leftArm', item.left_arm,
        'rightArm', item.right_arm,
        'leftThigh', item.left_thigh,
        'rightThigh', item.right_thigh,
        'notes', item.notes
      ) order by item.record_date desc, item.id desc)
      from visible item
    ), '[]'::jsonb),
    'hasMore', (select has_more from page_state),
    'nextCursor', case when (select has_more from page_state) then (
...
-- Add only missing equivalent indexes. Dynamic DDL is limited to conditional index creation.
do $v41d_indexes$
declare
...
begin
  ...
  select exists (
    select 1
    from pg_catalog.pg_index index_record
    where index_record.indrelid = 'public.meals'::regclass
      and index_record.indisvalid
      and index_record.indpred is null
      and (
        select pg_catalog.array_agg(attribute.attname::text order by key_column.ordinality)
        ...
        where key_column.ordinality <= index_record.indnkeyatts
          and key_column.ordinality <= 3
      ) = array['user_id', 'meal_date', 'id']::text[]
  ) into equivalent_exists;
  if not equivalent_exists then
    if pg_catalog.to_regclass('public.meals_user_date_id_v41d_idx') is not null then
      raise exception 'v41d_index_name_conflict: public.meals_user_date_id_v41d_idx';
    end if;
    execute 'create index meals_user_date_id_v41d_idx on public.meals (user_id, meal_date desc, id desc)';
    execute 'comment on index public.meals_user_date_id_v41d_idx is ''FORJA V4.1D professional monitoring''';
  end if;

  select exists (
    select 1
    from pg_catalog.pg_index index_record
    where index_record.indrelid = 'public.evolution'::regclass
      and index_record.indisvalid
      and index_record.indpred is null
      and (
        select pg_catalog.array_agg(attribute.attname::text order by key_column.ordinality)
        ...
        where key_column.ordinality <= index_record.indnkeyatts
          and key_column.ordinality <= 3
      ) = array['user_id', 'record_date', 'id']::text[]
  ) into equivalent_exists;
  if not equivalent_exists then
    if pg_catalog.to_regclass('public.evolution_user_date_id_v41d_idx') is not null then
      raise exception 'v41d_index_name_conflict: public.evolution_user_date_id_v41d_idx';
    end if;
    execute 'create index evolution_user_date_id_v41d_idx on public.evolution (user_id, record_date desc, id desc)';
    execute 'comment on index public.evolution_user_date_id_v41d_idx is ''FORJA V4.1D professional monitoring''';
  end if;
end;
$v41d_indexes$;

revoke all on function public.assert_professional_student_read_access_v41d(uuid, uuid, text, text, uuid) from public, anon, authenticated;
...
revoke all on function public.list_my_student_workout_executions(uuid, uuid, uuid, date, date, integer, date, uuid) from public, anon, authenticated;
revoke all on function public.list_my_student_nutrition_logs(uuid, uuid, uuid, date, date, integer, date, uuid) from public, anon, authenticated;
revoke all on function public.list_my_student_evolution(uuid, uuid, text, uuid, date, date, integer, date, uuid) from public, anon, authenticated;

grant execute on function public.list_my_student_workout_executions(uuid, uuid, uuid, date, date, integer, date, uuid) to authenticated;
grant execute on function public.list_my_student_nutrition_logs(uuid, uuid, uuid, date, date, integer, date, uuid) to authenticated;
grant execute on function public.list_my_student_evolution(uuid, uuid, text, uuid, date, date, integer, date, uuid) to authenticated;
