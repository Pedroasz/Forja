      ...
      where metadata_item.key not in ('professionalType','version','effectiveFrom','severity','action','route','code')
        or case metadata_item.key
          ...
          when 'effectiveFrom' then
            ...
            or (jsonb_typeof(metadata_item.value)='string' and metadata_item.value#>>'{}'!~'^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$')
          when 'severity' then
            jsonb_typeof(metadata_item.value)<>'string'
...
create table if not exists public.user_notifications (
  ...
  constraint user_notifications_dedupe_key_check check (dedupe_key is null or char_length(dedupe_key) between 1 and 200),
...
drop policy if exists user_notifications_select_own_v41c on public.user_notifications;
...
set search_path = ''
as $$
declare notification_id uuid; safe_metadata jsonb:=coalesce(target_metadata,'{}'::jsonb);
begin
  ...
  if target_dedupe_key is not null and char_length(target_dedupe_key) not between 1 and 200 then raise exception 'invalid_notification_dedupe_key' using errcode='22023'; end if;
  ...
  values(target_recipient_user_id,target_actor_user_id,target_notification_type,btrim(target_title),btrim(target_message),target_entity_type,target_entity_id,target_dedupe_key,safe_metadata)
  on conflict (recipient_user_id,dedupe_key) where dedupe_key is not null do nothing
  returning id into notification_id;

  if notification_id is null and target_dedupe_key is not null then
    select notification.id into notification_id from public.user_notifications notification
    where notification.recipient_user_id=target_recipient_user_id and notification.dedupe_key=target_dedupe_key;
  end if;
...
begin
  ...
  if target_limit not between 1 and 100 then raise exception 'invalid_notification_limit' using errcode='22023'; end if;
...
revoke all on function public.notification_metadata_is_safe_v41c(jsonb),public.create_user_notification_v41c(uuid,uuid,text,text,text,text,uuid,text,jsonb),public.notify_relationship_status_v41c(),public.notify_workout_assignment_v41c() from public,anon,authenticated;
